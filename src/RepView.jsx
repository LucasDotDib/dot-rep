...
import {
  C, ipt, Btn, FormPDV, PdvCardLight, BottomNav, HeroBanner,
...
export default function RepView({ onLogout }) {
  ...
  const filterBtn = (active) => ({
    ...
    borderRadius:7, fontFamily:font, whiteSpace:"nowrap",
    ...
    <div style={wrap}>
      ...
      <div style={header}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            <p style={{ margin:0, fontSize:10, color:C.muted, letterSpacing:".06em", textTransform:"uppercase" }}>Dot Energy</p>
            ...
            <>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {listaHoje.length === 0 ? (
                  ...
                  <PdvCardLight key={s.id} s={s} {...cardProps} />
                ))}
              </div>

              {totalRota > 0 && visitadosRota === totalRota && (
                <div style={{ marginTop:12, padding:14, background:C.greenDim, border:`1px solid ${C.greenBorder}`, borderRadius:16, textAlign:"center" }}>
        ...
        <div style={pad}>
          {showAdd && (
            <div style={{ ...card, padding:16, marginBottom:14 }}>
              ...
              <FormPDV
                initial={{ nome:"", end:"", cep:"", tipo:"facu", prio:0, rotaId:null }}
                onSave={adicionar} onCancel={() => setShowAdd(false)} saving={saving} rotas={rotas}
              />
            </div>
          )}
          ...
          <div style={{ position:"relative", marginBottom:10 }}>
            ...
            <input
              ...
              style={{ ...ipt, paddingLeft:34, background:C.white }}
            />
          </div>
          ...
          <div style={{ display:"flex", gap:5, marginBottom:10, overflowX:"auto" }}>
            {[["todos","Todos"],["prio","⭐ Prior."],["pendentes","Pendentes"],["hoje","Hoje"]].map(([v,l]) => (
              <button key={v} onClick={() => setFilter(v)} style={filterBtn(filter===v)}>{l}</button>
            ))}
          </div>
        ...
        return (
          <div style={pad}>
            ...
            <div style={{ background:"#3c3489", borderRadius:16, padding:"16px 18px", marginBottom:14, color:"#fff" }}>
              <div style={{ fontSize:10, color:"rgba(255,255,255,.5)", letterSpacing:".06em", textTransform:"uppercase", marginBottom:6 }}>
                Displays consignados
              </div>
              ...
              <div style={{ fontSize:12, color:"rgba(255,255,255,.5)" }}>
                PDV{consignados.length!==1?"s":""} com display deixado para consignação
              </div>
            </div>
