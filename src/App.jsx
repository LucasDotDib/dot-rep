...
export default function App() {
  ...
  const [form, setForm]             = useState({ nome:"", end:"", tipo:"facu", prio:0 });
  ...
  const adicionar = useCallback(async () => {
    ...
    const { error } = await supabase.from("pdvs").insert([{
      ...
      endereco:   form.end || "",
      tipo:       form.tipo,
      prioridade: form.prio,
      vendeu_dot: false,
      ultima_visita: null,
      obs: "",
    }]);
    if (error) setErro(error.message);
    else { setForm({ nome:"", end:"", tipo:"facu", prio:0 }); setShowAdd(false); }
    setSaving(false);
  }, [form, saving]);
  ...
  const remover = useCallback(async (id) => {
    ...
    if (error) setErro(error.message);
    else { setExpanded(null); setConfirmDel(null); }
  }, []);
  ...
  if (erro) return (
    <div style={{ background:C.bg, color:C.red, minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:12, fontFamily:"system-ui", padding:"2rem", textAlign:"center" }}>
  ...
  const hoje  = stores.filter(s => daysSince(s.visita) === 0).length;
  const pend  = stores.filter(s => daysSince(s.visita) !== 0).length;
  const ORDER = { nunca:0, atrasado:1, recente:2, ok:3 };

  const lista = stores
    .filter(s => {
      const q = search.toLowerCase();
      const m = !q || s.nome.toLowerCase().includes(q) || (s.end||"").toLowerCase().includes(q);
    ...
    .sort((a,b) => b.prio - a.prio || ORDER[visitStatus(a.visita)] - ORDER[visitStatus(b.visita)]);
    ...
    <div style={{ fontFamily:"'SF Pro Display',-apple-system,BlinkMacSystemFont,sans-serif", background:C.bg, color:C.white, minHeight:"100vh", maxWidth:440, margin:"0 auto", paddingBottom:"2rem" }}>
      ...
      <div style={{ padding:"1.5rem 1rem 1rem", borderBottom:`1px solid ${C.border}` }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
          ...
          <Btn variant={showAdd?"danger":"orange"} style={{ padding:"9px 16px" }} onClick={() => { setShowAdd(v=>!v); setForm({ nome:"", end:"", tipo:"facu", prio:0 }); }}>
      ...
      {showAdd && (
        <div style={{ margin:"1rem", padding:"1.25rem", background:C.surface, border:`1px solid ${C.border}`, borderRadius:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
            <input placeholder="Nome do estabelecimento *" value={form.nome} onChange={e=>setForm(f=>({...f,nome:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&adicionar()} style={ipt} autoFocus />
            <input placeholder="Endereço (opcional)" value={form.end} onChange={e=>setForm(f=>({...f,end:e.target.value}))} style={ipt} />
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:9 }}>
              <select value={form.tipo} onChange={e=>setForm(f=>({...f,tipo:e.target.value}))} style={{...ipt,padding:"11px 10px"}}>
                {TIPOS.map(t=><option key={t} value={t}>{TIPO_LABEL[t]}</option>)}
              </select>
              <Btn variant={form.prio===1?"orange":"ghost"} style={{padding:"11px 0"}} onClick={()=>setForm(f=>({...f,prio:f.prio===1?0:1}))}>
                {form.prio===1?"⭐ Prior.":"☆ Prior."}
              </Btn>
            </div>
            <Btn variant={form.nome.trim()&&!saving?"orange":"ghost"} style={{padding:"13px 0",fontSize:14,opacity:form.nome.trim()?1:0.4}} onClick={adicionar}>
              {saving?"Salvando…":"Adicionar PDV"}
            </Btn>
          </div>
        </div>
      )}
      ...
      <div style={{ padding:"1rem 1rem 0.75rem" }}>
        <input type="text" placeholder="Buscar PDV…" value={search} onChange={e=>setSearch(e.target.value)} style={{...ipt,marginBottom:10}} />
      ...
      <div style={{ padding:"0 1rem", display:"flex", flexDirection:"column", gap:8 }}>
        ...
        {lista.length===0&&total>0&&(
          <div style={{ textAlign:"center", padding:"2rem 0", color:C.gray, fontSize:14 }}>Nenhum PDV encontrado.</div>
        )}
        {lista.map(s => {
          const vs   = visitStatus(s.visita);
          const cfg  = STATUS[vs];
          const days = s.visita ? daysSince(s.visita) : null;
          const isExp=expanded===s.id, isFlash=flash===s.id, isDel=confirmDel===s.id, isOk=vs==="ok";
          ...
          return (
            <div key={s.id} style={{ background:C.surface, border:`1px solid ${C.border}`, borderLeft:`3px solid ${s.prio===1?C.orange:C.border}`, borderRadius:12, overflow:"hidden" }}>
              ...
              <div style={{ display:"flex", gap:6, padding:"10px 14px" }}>
                ...
                <Btn variant="ghost" style={{padding:"11px 10px",fontSize:13}} onClick={()=>{setExpanded(isExp?null:s.id);setConfirmDel(null);}}>
                  {isExp?"▲":"▼"}
                </Btn>
              </div>
              {isExp&&(
                <div style={{ padding:"12px 14px 14px", borderTop:`1px solid ${C.border}` }}>
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    {s.end&&<div><div style={{ fontSize:10, color:C.gray, letterSpacing:"0.08em", marginBottom:4 }}>ENDEREÇO</div><div style={{ fontSize:13, color:C.white }}>{s.end}</div></div>}
                    <div>
                    ...
                    {isDel?(
                      <div style={{ display:"flex", gap:6 }}>
                        <Btn variant="danger" style={{flex:1,padding:"10px 0",fontSize:13}} onClick={()=>remover(s.id)}>Confirmar exclusão</Btn>
                        <Btn variant="ghost" style={{padding:"10px 14px",fontSize:13}} onClick={()=>setConfirmDel(null)}>Cancelar</Btn>
                      </div>
                    ):(
                      <Btn variant="danger" style={{padding:"9px 0",fontSize:12,width:"100%"}} onClick={()=>setConfirmDel(s.id)}>Remover PDV</Btn>
                    )}
                  </div>
                </div>
              )}
        ...
        <div style={{ margin:"1.25rem 1rem 0", padding:"12px 14px", background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, textAlign:"center" }}>
          <span style={{ fontSize:12, color:C.gray, fontFamily:"monospace" }}>
            {hoje===total
              ...
              :<><span style={{color:C.green}}>{hoje} visitados</span><span style={{color:C.grayDim}}> · </span><span style={{color:C.amber}}>{pend} pendentes</span><span style={{color:C.grayDim}}> · </span><span>{new Date().toLocaleDateString("pt-BR",{day:"numeric",month:"short"})}</span></>
            }
          </span>
        </div>
      )}
